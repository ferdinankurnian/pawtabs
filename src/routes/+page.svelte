<script>
    ...
    const DEFS = {
        clock: {
            ...
            row: 1,
            label: "Clock",
            bgVariants: ["dark", "light"],
        },
        search: {
            col: 2,
            row: 1,
            label: "Search",
            variants: { "1x1": { col: 1, row: 1 }, "2x1": { col: 2, row: 1 } },
        },
        focus: {
            col: 2,
            row: 1,
        ...
        routine: {
            ...
            row: 2,
            label: "Routine",
            variants: { "2x1": { col: 2, row: 1 }, "2x2": { col: 2, row: 2 } },
        },
        todo: {
            col: 1,
            row: 2,
        ...
        >
            {#each widgets as w (w.id)}
                ...
                <div
                    ...
                    style="grid-column: span {variant?.col ??
                        1}; grid-row: span {variant?.row ?? 1};"
                >
                    {#if editMode}
                        {#if w.id === "links"}
                        ...
                        {#if w.id === "focus"}
                            <div
                                class="variant-picker"
                                role="group"
                                aria-label="Focus size"
                            >
                                <button
                                    class="variant-btn"
                                    class:active={(w.variant ?? "2x1") ===
                                        "1x1"}
                                    on:click|stopPropagation={() => {
                                        widgets = widgets.map((item) =>
                                            item.id === "focus"
                        ...
                        {#if w.id === "todo"}
                            <div
                                class="variant-picker"
                                role="group"
                                aria-label="To Do size"
                            >
                                <button
                                    class="variant-btn"
                                    class:active={(w.variant ?? "1x2") ===
                                        "1x1"}
                                    on:click|stopPropagation={() => {
                                        widgets = widgets.map((item) =>
                                            item.id === "todo"
                        ...
                        {#if w.id === "routine"}
                            <div
                                class="variant-picker"
                                role="group"
                                aria-label="Routine size"
                            >
                                <button
                                    class="variant-btn"
                                    class:active={(w.variant ?? "2x2") ===
                                        "2x1"}
                                    on:click|stopPropagation={() => {
                                        widgets = widgets.map((item) =>
                                            item.id === "routine"
                    ...
                    <div class="widget-content" class:blurred={editMode}>
                        ...
                        {:else if w.id === "search"}
                            <SearchWidget
                                autoFocus={$store.searchFocusOnOpen ?? true}
                                variant={w.variant ?? "1x1"}
                            />
